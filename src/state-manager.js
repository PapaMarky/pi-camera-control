...
export class NetworkStateManager extends EventEmitter {
  ...
  async ensureAccessPointRunning() {
    try {
      ...
      const ap0Active =
        this.networkState.interfaces.get("ap0")?.active || false;
      const hostapdActive =
        this.networkState.services.get("hostapd")?.active || false;
      ...
      const verifyAp0Active =
        this.networkState.interfaces.get("ap0")?.active || false;
      const verifyHostapdActive =
        this.networkState.services.get("hostapd")?.active || false;
      ...
      if (verifyHostapdActive && verifyAp0Active) {
        ...
        logger.error(
          "Failed to start Access Point - camera controller may be unreachable",
        );
        this.emit("accessPointEnsured", {
          active: false,
  ...
  async detectCurrentMode() {
    try {
      ...
      const wlan0Active =
        this.networkState.interfaces.get("wlan0")?.active || false;
      const ap0Active =
        this.networkState.interfaces.get("ap0")?.active || false;
      const hostapdActive =
        this.networkState.services.get("hostapd")?.active || false;
      ...
      if (hostapdActive && ap0Active && wlan0Active) {
        ...
        logger.info(
          "WiFi client active, AP will be started during initialization",
        );
        this.currentMode = "development";
      } else {
        ...
        this.currentMode = "field";
        logger.info(
          `Network services not yet active, defaulting to ${this.currentMode} mode`,
        );
      }
  ...
  async switchMode(targetMode) {
    if (!["field", "development"].includes(targetMode)) {
      throw new Error(
        `Invalid network mode: ${targetMode}. Valid modes are 'field' and 'development'`,
      );
    }
  ...
  async updateInterfaceStates() {
    ...
    for (const iface of interfaces) {
      try {
        ...
        logger.debug(
          `Failed to get state for interface ${iface}:`,
          error.message,
        );
        this.updateInterfaceState(iface, {
          active: false,
          error: error.message,
        });
      }
  ...
  async updateServiceStates() {
    ...
    for (const service of services) {
      try {
        ...
        logger.debug(
          `Failed to get state for service ${service}:`,
          error.message,
        );
