...
describe("WebSocket Handler Unit Tests", () => {
  ...
  describe("Message Routing", () => {
    ...
    test("routes network_scan message correctly", async () => {
      ...
      expect(
        mockNetworkManager.serviceManager.scanWiFiNetworks,
    ...
    test("routes network_connect message correctly", async () => {
      ...
      expect(
        mockNetworkManager.serviceManager.connectToWiFi,
  ...
  describe("Error Handling", () => {
    ...
    test("handles camera operation failure", async () => {
      ...
      expect(sentMessages[0].error.message).toBe(
        "Failed to take photo: Camera communication failed",
      );
  ...
  describe("Timelapse Report Handlers", () => {
    ...
    test("handles update_report_title", async () => {
      ...
      mockIntervalometerStateManager.updateReportTitle.mockResolvedValue(
        updatedReport,
      );
      ...
      ).toHaveBeenCalledWith("test-id", "Updated Title");
    ...
    test("validates required fields for report operations", async () => {
      ...
      expect(sentMessages[0].type).toBe("error");
      expect(sentMessages[0].error.message).toBe(
        "Report ID and title are required",
      );
