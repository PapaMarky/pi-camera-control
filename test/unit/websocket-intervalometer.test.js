...
describe("WebSocket Intervalometer Handler Tests", () => {
  ...
  describe("Intervalometer Session Management", () => {
    ...
    test("starts intervalometer with stop time", async () => {
      ...
      const createSessionCall =
        mockIntervalometerStateManager.createSession.mock.calls[0];
    ...
    test("prevents starting when session already running", async () => {
      ...
      expect(sentMessages[0].error.message).toBe(
    ...
    test("handles validation failure on start", async () => {
      ...
      expect(sentMessages[0].type).toBe("error");
      expect(sentMessages[0].error.message).toBe(
        "Interval conflicts with current shutter speed",
      );
    });
    ...
    test("handles stop when no session running", async () => {
      ...
      expect(sentMessages[0].type).toBe("error");
      expect(sentMessages[0].error.message).toBe(
        "No intervalometer session is running",
      );
    });
  ...
  describe("Session Event Handling", () => {
    ...
    test("handles session started event", async () => {
      ...
      const startedHandler = mockSession.on.mock.calls.find(
        (call) => call[0] === "started",
      )[1];
    ...
    test("handles session completed event", async () => {
      ...
      const completedHandler = mockSession.on.mock.calls.find(
      ...
      expect(sentMessages[1].eventType).toBe(
  ...
  describe("Report Management", () => {
    ...
    test("saves session as report", async () => {
      ...
      expect(
        mockIntervalometerStateManager.saveSessionReport,
      ).toHaveBeenCalledWith("session-123", "Saved Session Report");
    ...
    test("discards session", async () => {
      ...
      expect(
        mockIntervalometerStateManager.discardSession,
    ...
    test("deletes timelapse report", async () => {
      ...
      expect(mockIntervalometerStateManager.deleteReport).toHaveBeenCalledWith(
        "report-1",
      );
      // Dual emission: direct response + timelapse_event broadcast
      expect(sentMessages).toHaveLength(2);
  ...
  describe("Error Validation", () => {
    ...
    test("handles intervalometer state manager not available", async () => {
      ...
      const messageHandler = newMockWs.on.mock.calls.find(
        (call) => call[0] === "message",
      )[1];
      ...
      expect(sentMessages[0].type).toBe("error");
      expect(sentMessages[0].error.message).toBe(
        "Timelapse reporting not available",
      );
    });
  });
});
