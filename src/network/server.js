...
import { StartupChecks } from "./system/startup-checks.js";
...
class CameraControlServer {
  ...
  async start() {
    try {
      // Run system startup checks and fixes
      logger.info("Running system startup checks...");
      const startupChecks = new StartupChecks();
      const checkResults = await startupChecks.runAllChecks();

      if (checkResults.failedChecks > 0) {
        logger.warn(`${checkResults.failedChecks} startup checks had issues, but continuing startup`);
      } else {
        logger.info("All system startup checks passed");
      }
