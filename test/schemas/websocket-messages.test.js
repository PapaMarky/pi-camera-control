    ...
    for (const [key, expectedType] of Object.entries(schemaObj)) {
      ...
      if (typeof expectedType === 'object' && !Array.isArray(expectedType)) {
        ...
        if (!obj || !obj.hasOwnProperty(key)) {
          ...
          errors.push(`Type mismatch at ${currentPath}: expected ${expectedType}, got ${typeof obj[key]}`);
        }
      }
    }
...
describe('WebSocket Message Schema Validation', () => {
  describe('Server Message Schemas', () => {
    ...
    test('welcome message follows schema', () => {
      ...
      const errors = validateSchema(welcome, MessageSchemas.serverMessages.welcome);
      expect(errors).toEqual([]);
    });
  ...
  describe('Client Message Schemas', () => {
    test('start_intervalometer_with_title message follows schema', () => {
      ...
      const errors = validateSchema(message, MessageSchemas.clientMessages.start_intervalometer_with_title);
      expect(errors).toEqual([]);
    });
    ...
    test('network_connect message follows schema', () => {
      ...
      const errors = validateSchema(message, MessageSchemas.clientMessages.network_connect);
      expect(errors).toEqual([]);
    });
  });
