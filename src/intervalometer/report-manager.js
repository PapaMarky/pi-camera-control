...
export class TimelapseReportManager {
  constructor(options = {}) {
    ...
    this.storageDir =
      options.storageDir || join(process.cwd(), "data", "timelapse-reports");
  ...
  async loadReportsIntoCache() {
    try {
      ...
      const reportFiles = files.filter((file) =>
        file.endsWith(this.reportExtension),
      );
      ...
      for (const file of reportFiles) {
        try {
          ...
          logger.warn("Failed to load report file:", {
            file,
            error: error.message,
          });
        }
  ...
  validateReport(report) {
    ...
    for (const field of requiredFieldsV1) {
      if (!report.hasOwnProperty(field)) {
        logger.debug("Report missing required field:", {
          field,
          reportId: report.id,
        });
        return false;
  ...
  async loadReports() {
    try {
      ...
      const reports = Array.from(this.reportCache.values()).map(
        (item) => item.report,
      );
  ...
  async updateReportTitle(reportId, newTitle) {
    try {
      ...
      logger.error("Failed to update report title:", {
        reportId,
        newTitle,
        error,
      });
    ...
    try {
      ...
      for (const cached of this.reportCache.values()) {
        try {
          ...
          logger.debug("Could not get file size:", {
    ...
    try {
      ...
      reportsToDelete = reportsToDelete.concat(
        sortedReports.filter(
          (report) => new Date(report.startTime) < cutoffDate,
        ),
      );
      ...
      if (dryRun) {
        ...
        return {
          totalReports: reports.length,
          reportsToDelete: uniqueReportsToDelete.length,
          reports: uniqueReportsToDelete.map((r) => ({
            id: r.id,
            title: r.title,
      ...
      for (const report of uniqueReportsToDelete) {
        try {
          ...
          logger.warn("Failed to delete report during cleanup:", {
            reportId: report.id,
            error,
          });
        }
