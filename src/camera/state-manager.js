...
export class CameraStateManager extends EventEmitter {
  ...
  async registerCamera(uuid, deviceInfo) {
    ...
    const ipChanged =
      existingCamera && existingCamera.info.ipAddress !== deviceInfo.ipAddress;
    ...
    if (lastIP && lastIP !== deviceInfo.ipAddress) {
      ...
      if (uuid === this.primaryCameraUuid && this.primaryController) {
        ...
        logger.debug(
          `IP changed for non-primary camera ${uuid} or no primary controller available`,
        );
        logger.debug(
          `primaryCameraUuid: ${this.primaryCameraUuid}, hasController: ${!!this.primaryController}`,
        );
      }
    }
    ...
    if (shouldAutoConnect) {
      logger.info(
        `Auto-connecting to camera ${uuid} (${deviceInfo.modelName}): ${shouldAutoConnect.reason}`,
      );
      try {
      ...
      logger.debug(
        `Not auto-connecting to camera ${uuid}: primary=${this.primaryCameraUuid}, connected=${deviceInfo.connected}`,
      );
    }
  ...
  async deduplicateCamera(newUuid, newDeviceInfo) {
    ...
    for (const [existingUuid, existingCamera] of this.cameras) {
      ...
      if (
        newDeviceInfo.serialNumber &&
        existingInfo.serialNumber &&
        newDeviceInfo.serialNumber === existingInfo.serialNumber
      ) {
        logger.info(
          `Found duplicate camera by serial number: ${existingUuid} -> ${newUuid}`,
        );
      ...
      if (
        newDeviceInfo.ipAddress === existingInfo.ipAddress &&
        newDeviceInfo.modelName &&
        existingInfo.modelName &&
        newDeviceInfo.modelName.includes("Canon") &&
        existingInfo.modelName.includes("Canon")
      ) {
        // Prefer UPnP discovered cameras over manual/IP-scan entries
        const newIsUPnP =
          !newDeviceInfo.isManual && !newDeviceInfo.discoveryMethod;
        const existingIsUPnP =
          !existingInfo.isManual && !existingInfo.discoveryMethod;

        if (newIsUPnP && !existingIsUPnP) {
          logger.info(
            `Found duplicate camera, preferring UPnP entry: ${existingUuid} -> ${newUuid}`,
          );
          duplicatesToRemove.push(existingUuid);
        } else if (!newIsUPnP && existingIsUPnP) {
          logger.info(
            `Skipping duplicate manual/scan entry, keeping UPnP: ${newUuid} (keeping ${existingUuid})`,
          );
  ...
  shouldAutoConnect(uuid, _deviceInfo) {
    ...
    if (connectionHistory.length > 0) {
      const hadSuccessfulConnection = connectionHistory.some(
        (event) => event.event === "connected" || event.event === "reconnected",
      );
  ...
  async connectToIP(ip, port = "443") {
    ...
    if (existingUuid) {
      logger.info(
        `IP ${ip} matches known camera ${existingUuid}, using existing entry`,
      );
  ...
  async reconnectPrimaryCamera(newIP, newPort = "443") {
    ...
    logger.info(
      `Reconnecting primary camera ${uuid} to new IP ${newIP}:${newPort}`,
    );
    ...
    try {
      ...
      const controller = new CameraController(newIP, newPort, (status) =>
        this.handleControllerStatusChange(uuid, status),
      );
      ...
      if (success) {
        ...
        logger.info(
          `Primary camera ${uuid} successfully reconnected to ${newIP}:${newPort}`,
        );
      ...
      logger.error(
        `Failed to reconnect primary camera to ${newIP}:${newPort}:`,
        error,
      );
      cameraData.status = "failed";
      cameraData.lastError = error.message;
