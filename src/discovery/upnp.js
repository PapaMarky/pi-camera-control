...
export class UPnPDiscovery extends EventEmitter {
  ...
  async startDiscovery() {
    try {
      ...
      if (interfaces.length === 0) {
        logger.warn(
          "No network interfaces available - UPnP discovery will start but may have limited functionality",
        );
      }
      ...
      if (this.socket) {
        try {
          ...
          logger.debug(
            "Error closing socket during cleanup:",
            closeError.message,
          );
        }
        this.socket = null;
  ...
  joinMulticastOnInterfaces() {
    ...
    logger.debug(
      ...
      interfaces.map((i) => `${i.name}:${i.address}`),
    );
    ...
    for (const iface of interfaces) {
      try {
        ...
        logger.info(
          `Joined multicast on interface ${iface.name} (${iface.address})`,
        );
        joinedCount++;
      } catch (error) {
        ...
        if (error.code === "ENODEV" || error.code === "EADDRNOTAVAIL") {
          logger.warn(
            `Interface ${iface.name} (${iface.address}) not available for multicast: ${error.message}`,
          );
        } else {
          logger.warn(
            `Failed to join multicast on interface ${iface.name} (${iface.address}):`,
            error.message,
          );
        }
      }
    ...
    if (joinedCount === 0) {
      try {
        ...
        if (error.code === "ENODEV") {
          logger.warn(
            "No network devices available for multicast - UPnP discovery will have limited functionality",
          );
        } else {
          logger.warn(
            "Failed to join multicast group on default interface:",
            error.message,
          );
        }
      }
    ...
    if (joinedCount === 0) {
      logger.warn(
        "UPnP discovery: No multicast interfaces available - passive discovery only",
      );
    } else {
      logger.info(
        `UPnP discovery: Successfully joined multicast on ${joinedCount} interface(s)`,
      );
    }
  }
      ...
      for (const iface of interfaces) {
        try {
          ...
          await new Promise((resolve, _reject) => {
            // TODO: Handle network errors properly instead of ignoring reject
            this.socket.send(
              searchMessage,
              this.MULTICAST_PORT,
              ...
              (error) => {
                if (error) {
                  logger.warn(
                    `M-SEARCH failed for ${serviceType} on ${iface.name}:`,
                    error,
                  );
                  resolve(); // Don't fail the whole process
                } else {
                  logger.debug(
                    `M-SEARCH sent for ${serviceType} on ${iface.name} (${iface.address})`,
                  );
                  resolve();
                }
              },
          ...
          logger.warn(
            `Error sending M-SEARCH on ${iface.name}:`,
            error.message,
          );
        }
      }
          ...
          this.socket.send(
            ...
            (error) => {
              if (error) {
                ...
                logger.debug(
                  `M-SEARCH sent for ${serviceType} on default interface`,
                );
                resolve();
              }
            },
  ...
  handleSSDPMessage(message, remote) {
    try {
      logger.info(
        `Received SSDP message from ${remote.address}:${remote.port}:`,
      );
  ...
  handleNotifyMessage(lines, remote) {
    ...
    logger.debug(
      `NOTIFY message from ${remote.address}: NT=${nt}, USN=${usn}, NTS=${nts}, Location=${location}`,
    );
    ...
    if (nts === "ssdp:alive") {
      ...
      if (isCanon) {
        logger.info(`Canon camera NOTIFY from ${remote.address}:`, {
          nt,
          usn,
          location,
        });
  ...
  handleSearchResponse(lines, remote) {
    ...
    logger.debug(
      `M-SEARCH response from ${remote.address}: ST=${st}, USN=${usn}, Location=${location}`,
    );
    ...
    if (isCanon) {
      logger.info(`Canon camera M-SEARCH response from ${remote.address}:`, {
        st,
        usn,
        location,
      });
  ...
  isCanonCamera(serviceType, usn) {
    ...
    const hasCanonIndicator = canonServiceTypes.some(
      (indicator) =>
    ...
    const isCanonRootDevice =
      serviceType === "upnp:rootdevice" &&
      usn &&
      canonServiceTypes.some((indicator) => usn.includes(indicator));

    logger.debug(
      `Canon camera check - ST: ${serviceType}, USN: ${usn}, hasCanonIndicator: ${hasCanonIndicator}, isCanonRootDevice: ${isCanonRootDevice}`,
    );
  ...
  async processDeviceLocation(headers, remote) {
    ...
    try {
      ...
      if (deviceInfo) {
        ...
        logger.info(
          `Canon camera discovered: ${deviceInfo.modelName} (${deviceInfo.serialNumber})`,
          {
      ...
      logger.error(
        `Failed to fetch device description from ${location}:`,
        error.message,
      );
    }
  }
  ...
  parseDeviceDescription(deviceDesc) {
    try {
      ...
      for (const prefix of nsPrefixes) {
        xOnService =
          xOnService || this.findExtendedValue(service, "X_onService", prefix);
        xAccessURL =
          xAccessURL || this.findExtendedValue(service, "X_accessURL", prefix);
        xDeviceNickname =
          xDeviceNickname ||
          this.findExtendedValue(service, "X_deviceNickname", prefix);
      }
      ...
      if (!deviceInfo.ccapiUrl || !deviceInfo.manufacturer?.includes("Canon")) {
        logger.debug(
          "Device description does not appear to be a Canon camera with CCAPI",
        );
        return null;
      }
    ...
    this.searchTimeout = setTimeout(
      async () => {
        try {
          ...
          if (interfaces.length === 0) {
            logger.debug(
              "No network interfaces available for periodic M-SEARCH, skipping...",
            );
          ...
          if (
            this.searchFailureCount <= 3 ||
            this.searchFailureCount % 10 === 0
          ) {
            logger.warn(
              `Periodic M-SEARCH failed (${this.searchFailureCount} times):`,
              error.message,
            );
          }
        }
        ...
        const backoffMultiplier = Math.min(
          Math.pow(2, Math.min(this.searchFailureCount, 4)),
          5,
        ); // Max 5x = 5 minutes
        ...
        if (this.searchFailureCount > 0) {
          logger.debug(
            `Scheduling next M-SEARCH in ${nextInterval / 1000} seconds (backoff due to ${this.searchFailureCount} failures)`,
          );
        }
      ...
      this.searchFailureCount > 0
        ? Math.min(
            60000 * Math.pow(2, Math.min(this.searchFailureCount, 4)),
            300000,
          )
        : 60000,
    );
