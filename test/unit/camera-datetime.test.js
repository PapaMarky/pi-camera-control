...
describe("Camera DateTime CCAPI", () => {
  ...
  describe("getCameraDateTime", () => {
    ...
    test("should throw error when camera not connected", async () => {
      ...
      await expect(controller.getCameraDateTime()).rejects.toThrow(
  ...
  describe("setCameraDateTime", () => {
      ...
      expect(mockClient.put).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          datetime: expect.stringMatching(
            /\w{3}, \d{2} \w{3} 2024 \d{2}:\d{2}:\d{2} [+-]\d{4}/,
          ),
    ...
    test("should handle timezone offsets", async () => {
      ...
      expect(mockClient.put).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          datetime: expect.stringMatching(
            /\w{3}, \d{2} \w{3} 202[34] \d{2}:\d{2}:\d{2} [+-]\d{4}/,
          ),
          dst: expect.any(Boolean),
        }),
      );
    ...
    test("should throw error when camera not connected", async () => {
      ...
      await expect(controller.setCameraDateTime(testDate)).rejects.toThrow(
        "Camera not connected",
      );
