  ...
  intervalometerStatus: {
    active: {
      ...
      properties: {
        ...
        stats: {
          type: 'object',
          required: ['startTime', 'shotsTaken', 'shotsSuccessful', 'shotsFailed', 'currentShot'],
          properties: {
      ...
      batterylist: {
        ...
        items: {
          ...
          properties: {
            ...
            kind: {
              type: 'string',
              enum: ['battery', 'not_inserted', 'ac_adapter', 'dc_coupler', 'unknown']
            },
            level: {
...
export function validateAPISchema(data, schemaName, subSchema = null) {
  const schema = subSchema ? APISchemas[schemaName][subSchema] : APISchemas[schemaName];

  if (!schema) {
    throw new Error(`Schema ${schemaName}${subSchema ? '.' + subSchema : ''} not found`);
  }
...
function validateField(value, schema, fieldName) {
  ...
  if (schema.type) {
    ...
    if (!expectedTypes.includes(actualType)) {
      errors.push(`Field ${fieldName}: expected ${expectedTypes.join(' or ')}, got ${actualType}`);
    }
  }
  ...
  if (typeof value === 'number') {
    if (schema.minimum !== undefined && value < schema.minimum) {
      errors.push(`Field ${fieldName}: value ${value} below minimum ${schema.minimum}`);
    }
    if (schema.maximum !== undefined && value > schema.maximum) {
      errors.push(`Field ${fieldName}: value ${value} above maximum ${schema.maximum}`);
    }
  }
