...
export class NetworkServiceManager extends EventEmitter {
  ...
  async checkSystemCapabilities() {
    ...
    for (const cmd of optionalCommands) {
      try {
        ...
        logger.warn(
          `Optional command not found: ${cmd} - some features may be limited`,
        );
      }
    }
  ...
  async startService(serviceName) {
    ...
    try {
      ...
      await execAsync(`systemctl start ${service.name}`, {
      ...
      this.emit("serviceStateChanged", {
        service: service.name,
        state: { active: false, error: error.message },
      });
      throw error;
    }
  }
  ...
  async stopService(serviceName) {
    ...
    try {
      ...
      await execAsync(`systemctl stop ${service.name}`, {
        timeout: this.operationTimeout,
      });
  ...
  async restartService(serviceName) {
    ...
    try {
      ...
      await execAsync(`systemctl restart ${service.name}`, {
        timeout: this.operationTimeout,
      });
  ...
  async getServiceState(serviceName) {
    try {
      const { stdout } = await execAsync(
        `systemctl show ${serviceName} --property=ActiveState,SubState,LoadState`,
      );
    ...
    try {
      ...
      if (iface === "wlan0") {
        ...
        try {
          ...
          logger.error(
            `Failed to get WiFi status for wlan0: ${error.message}`,
            error,
          );
        ...
        try {
          ...
          logger.debug(
            `ap0 enhanced with ssid: ${ssid}, clients: ${clients.length}, ip: ${baseState.ip}, status: ${baseState.status}`,
          );
        } catch (error) {
    ...
    try {
      ...
      if (forceRefresh) {
        try {
          ...
          logger.debug(
            "nmcli rescan failed, proceeding with existing scan data:",
            error.message,
          );
        }
      }
  ...
  async getSavedWiFiConnections() {
    try {
      ...
      const wifiConnections = connections
        .split("\n")
  ...
  async connectToWiFi(ssid, password, _priority = 1) {
    try {
      ...
      try {
        ...
        for (const line of lines) {
          if (line.includes("wifi") || line.includes("wireless")) {
            ...
            if (connectionName === ssid) {
              logger.info(
                `Removing existing NetworkManager connection: ${connectionName}`,
              );
        ...
        logger.debug(
          "Could not check/remove existing connections:",
          error.message,
        );
      }
      ...
      logger.debug(
        `Executing: ${connectCmd.replace(/password "[^"]*"/, 'password "***"')}`,
      );
      ...
      for (const line of lines) {
        if (line.includes("wifi") || line.includes("wireless")) {
          ...
          if (device === "wlan0" && connectionName === expectedSSID) {
            // Get signal strength
            try {
              const { stdout: signalInfo } = await execAsync(
  ...
  async getSavedNetworks() {
    try {
      ...
      for (const line of lines) {
        if (
          line.trim() &&
          (line.includes("wifi") || line.includes("wireless"))
        ) {
          const parts = line.split(":");
          if (parts.length >= 2) {
            const connectionName = parts[0];
            ...
            try {
              const { stdout: ssidOutput } = await execAsync(
                `nmcli con show "${connectionName}" | grep "802-11-wireless.ssid"`,
              );
              const ssidMatch = ssidOutput.match(
                /802-11-wireless\.ssid:\s*(.+)/,
              );
              const targetSSID = ssidMatch
                ? ssidMatch[1].trim()
                : connectionName;
              ...
              logger.debug(
                `Failed to get SSID for connection ${connectionName}:`,
                ssidError.message,
              );
              networks.push({
                name: connectionName,
  ...
  async getWiFiStatus() {
    try {
      ...
      for (const line of lines) {
        if (line.includes("wifi") || line.includes("wireless")) {
          ...
          if (device === "wlan0") {
            ...
            try {
              const { stdout: detailsOutput } = await execAsync(
                `nmcli -t -f IN-USE,SIGNAL,SSID dev wifi list`,
              );
      ...
      logger.debug(
        "NetworkManager status check failed, trying fallback methods:",
        error.message,
      );
      ...
      try {
        ...
        try {
          ...
          logger.error(
            "Failed to get WiFi status via iw, NetworkManager, and iwconfig:",
            error.message,
            fallbackError.message,
            iwconfigError.message,
          );
          return {
            connected: false,
            ssid: null,
  ...
  async getAPSSID() {
    try {
      const { stdout } = await execAsync(
        'grep -E "^ssid=" /etc/hostapd/hostapd.conf',
      );
  ...
  async setWiFiCountry(countryCode) {
    try {
      ...
      if (!countryCode || !/^[A-Z]{2}$/.test(countryCode)) {
        throw new Error(
          `Invalid country code format: ${countryCode}. Must be 2-letter ISO code (e.g., US, JP)`,
        );
      }

      logger.info(
        `Setting WiFi country to ${countryCode} for ALL wireless interfaces...`,
      );

      // First, let's check the current regulatory domain before making changes
      let initialCountry;
      try {
        const { stdout: regBefore } = await execAsync(
          `/usr/sbin/iw reg get | head -5`,
        );
        logger.info(`Current regulatory domain before change:\n${regBefore}`);
        initialCountry = await this.getWiFiCountry();
        logger.info(
          `Current country before change: ${JSON.stringify(initialCountry)}`,
        );
      } catch (error) {
      ...
      try {
        const { stdout } = await execAsync(
          `/usr/sbin/iw dev | grep Interface | awk '{print $2}'`,
        );
        ...
        logger.info(
          `Found wireless interfaces: ${JSON.stringify(wirelessInterfaces)}`,
        );
      } catch (error) {
      ...
      logger.info(
        `Setting global regulatory domain to ${countryCode} via iw...`,
      );
      try {
        const { stdout, stderr } = await execAsync(
          `/usr/sbin/iw reg set ${countryCode} 2>&1`,
        );
        ...
        const { stdout: regAfter } = await execAsync(
          `/usr/sbin/iw reg get | head -5`,
        );
        logger.info(`Regulatory domain after global change:\n${regAfter}`);
      } catch (iwError) {
        logger.error(
          "Failed to set global regulatory domain with iw:",
          iwError.message,
        );
      }
      ...
      for (const iface of wirelessInterfaces) {
        try {
          logger.info(
            `Setting country ${countryCode} for interface ${iface}...`,
          );
          ...
          try {
            ...
            logger.warn(
              `Interface ${iface} does not exist or cannot be brought up: ${linkError.message}`,
            );
            continue;
          }
          ...
          try {
            ...
            logger.warn(
              `Failed to set regulatory domain for ${iface}: ${regError.message}`,
            );
          }
          ...
          try {
            ...
            logger.debug(
              `Could not trigger scan on ${iface}: ${scanError.message}`,
            );
          }
        } catch (ifaceError) {
          logger.error(
            `Failed to process interface ${iface}: ${ifaceError.message}`,
          );
        }
      }
      ...
      try {
        ...
        const { stdout, stderr } = await execAsync(
          `raspi-config nonint do_wifi_country ${countryCode} 2>&1`,
        );
        if (stdout) logger.info(`raspi-config output: ${stdout}`);
        if (stderr) logger.warn(`raspi-config stderr: ${stderr}`);
        logger.info(
          `Set persistent WiFi country to ${countryCode} via raspi-config`,
        );
      } catch (raspiError) {
      ...
      try {
        ...
        logger.info(
          `Updating hostapd configuration with country ${countryCode}...`,
        );
        ...
        await execAsync(
          `echo '${newHostapdConfig}' | sudo tee ${hostapdPath} > /dev/null`,
        );
        ...
        logger.warn(
          "Failed to update hostapd configuration:",
          hostapdError.message,
        );
      }
      ...
      try {
        ...
        try {
          ...
          if (stdout.includes("country=")) {
            ...
            logger.info(
              "Replacing existing country setting in wpa_supplicant.conf",
            );
          } else {
            newConfig = stdout.replace(
              /(ctrl_interface=.*\n)/,
          ...
          await execAsync(
            `echo '${newConfig}' | sudo tee ${configPath} > /dev/null`,
          );
          logger.info(
            `Updated wpa_supplicant.conf with country ${countryCode}`,
          );
        } catch (fileError) {
          logger.info(
            `${configPath} does not exist or cannot be read: ${fileError.message}`,
          );
        }
      } catch (configError) {
        logger.error(
          "Failed to update wpa_supplicant.conf:",
          configError.message,
        );
      }
      ...
      try {
        ...
        logger.info(
          `Country verification result: ${JSON.stringify(verification)}`,
        );
        ...
        for (const iface of wirelessInterfaces) {
          try {
            const { stdout } = await execAsync(
              `/usr/sbin/iw dev ${iface} info | grep country || echo "No country info"`,
            );
            logger.info(`Interface ${iface} country info: ${stdout.trim()}`);
          } catch (ifaceCheckError) {
            logger.debug(
              `Could not check country for ${iface}: ${ifaceCheckError.message}`,
            );
          }
        }
        ...
        if (verification.country === countryCode) {
          logger.info(
            `WiFi country successfully changed to ${countryCode} for all interfaces`,
          );
          return {
            success: true,
            country: countryCode,
            message: `WiFi country changed to ${countryCode} for all interfaces`,
          };
        } else {
          logger.warn(
            `Country change verification failed. Expected: ${countryCode}, Got: ${verification.country}`,
          );
          return {
            success: true,
            country: countryCode,
            message: `Country set to ${countryCode} for all interfaces (verification pending)`,
          };
        }
      } catch (verifyError) {
        ...
        return {
          success: true,
          country: countryCode,
          message: `Country set to ${countryCode} for all interfaces (verification failed)`,
        };
      }
    } catch (error) {
  ...
  async getWiFiCountry() {
    try {
      ...
      try {
        const { stdout } = await execAsync(
          'grep -E "^country=" /etc/wpa_supplicant/wpa_supplicant.conf',
        );
        ...
        logger.debug(
          "Could not read country from wpa_supplicant.conf:",
          grepError.message,
        );
      }
