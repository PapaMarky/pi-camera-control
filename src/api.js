...
export function createApiRouter(
  ...
  router.post("/system/time", async (req, res) => {
    try {
      ...
      logger.info(
        `Time sync requested. Current: ${new Date().toISOString()}, Client: ${clientTime.toISOString()}`,
      );

      // Use child_process to set system time
      const { spawn } = await import("child_process");
      const setTime = spawn("sudo", ["date", "-s", formattedTime], {
        stdio: "pipe",
      });
  ...
  if (networkStateManager) {
    ...
    router.post("/network/mode", async (req, res) => {
      try {
        ...
        if (!mode || !["field", "development"].includes(mode)) {
          return res
            .status(400)
            .json({ error: 'Invalid mode. Must be "field" or "development"' });
        }
    ...
    router.post("/network/wifi/country", async (req, res) => {
      try {
        ...
        const result = await networkServiceManager.setWiFiCountry(
          country.toUpperCase(),
        );
        res.json(result);
      } catch (error) {
