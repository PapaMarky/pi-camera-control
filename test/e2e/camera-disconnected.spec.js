...
test.describe('Camera Disconnected State', () => {
  ...
  test('should show "-" for camera battery when disconnected', async ({ page }) => {
    await page.goto('/');
    await page.waitForLoadState('networkidle');
    await page.waitForTimeout(3000);
  ...
  test('should show "-" for camera mode when disconnected', async ({ page }) => {
    await page.goto('/');
    await page.waitForLoadState('networkidle');
    await page.waitForTimeout(3000);
  ...
  test('start intervalometer button should be disabled', async ({ page }) => {
    ...
    const isDisabled = await page.locator('#start-intervalometer-btn').isDisabled();
    expect(isDisabled).toBe(true);
  });
  ...
  test('intervalometer menu item should be disabled', async ({ page }) => {
    ...
    const intervalometerItem = await page.locator('button.menu-item[data-card="intervalometer"]');
  ...
  test('manual connect button should be visible when camera not found', async ({ page }) => {
    await page.goto('/');
    await page.waitForLoadState('networkidle');
